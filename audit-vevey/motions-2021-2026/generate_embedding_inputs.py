...
AUDIT = ROOT / "general-audit"
CHUNK_DIR = AUDIT / "chunks"
METADATA_DIR = AUDIT / "metadata"
OUTPUT = AUDIT / "embedding"
...
def main() -> None:
    ...
    for chunk_path in sorted(CHUNK_DIR.glob("*.json")):
        ...
        source_record, metadata_path = metadata[chunk_path.stem]
        base = source_record["document_metadata"]
        if base["processing_status"] != "validated":
            raise ValueError(
                f"Document non validé avec chunks: {base['document_id']}"
            )
        for chunk in chunks:
            ...
            records.append(
                {
                    ...
                    "content": content,
                    "content_hash": chunk.get("chunk_hash")
