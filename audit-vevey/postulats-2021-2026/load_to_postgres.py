...
def validate(inputs: list[dict], vectors: list[dict]) -> dict[str, dict]:
    ...
    if any(not row["document_id"].startswith("vevey_postulate_") for row in inputs):
        raise ValueError("Document hors périmètre Vevey/postulat")
    if any(row.get("category") != "postulat" for row in inputs):
        raise ValueError("Catégorie autre que postulat")
    return by_id
...
def document_metadata(row: dict, record: dict) -> dict:
    ...
    if (
        general.get("commune") != "Vevey"
        or general.get("category") != "postulat"
        or general.get("document_id") != row["document_id"]
    ):
        raise ValueError(f"Métadonnées hors périmètre: {row['document_id']}")
    return {
        **general,
...
def main() -> None:
    ...
    with psycopg.connect(database_url) as connection:
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT count(*) FROM documents WHERE NOT (document_id = ANY(%s))",
                (document_ids,),
            )
            unrelated_documents_before = cursor.fetchone()[0]
            cursor.execute(
                "SELECT count(*) FROM chunks WHERE NOT (document_id = ANY(%s))",
                (document_ids,),
            )
            unrelated_chunks_before = cursor.fetchone()[0]
            cursor.execute(
                "SELECT count(*) FROM documents WHERE document_id = ANY(%s)",
                (document_ids,),
            )
            ...
            for row, metadata, record in documents.values():
                specific = record["postulate_metadata"]
                            ...
                            json.dumps(
                                {
                                    ...
                                    "source_chunk_file": row["source_chunk_file"],
                                    "commune": "Vevey",
                                    "category": "postulat",
                                },
                                ensure_ascii=False,
            ...
            cursor.execute(
                "SELECT count(*) FROM documents WHERE document_id = ANY(%s) "
                "AND category='postulat' AND metadata->>'commune'='Vevey'",
                (document_ids,),
            )
            ...
            if actual != expected:
                raise RuntimeError(f"Chargement incomplet: {actual} != {expected}")
            if (
