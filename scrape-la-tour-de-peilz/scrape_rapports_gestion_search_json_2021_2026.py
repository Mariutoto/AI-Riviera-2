...
HEADERS = {"User-Agent": "AI-Riviera rapports-gestion faceted JSON importer"}
...
def parse_result_html(result_html: str, years: set[str] = YEARS) -> list[dict]:
    ...
    for match in faceted.RESULT_RE.finditer(result_html):
        ...
        if listing_year not in years:
            continue
        if source_title.casefold() != "rapport de gestion":
            continue
        if source_category.casefold() != "rapport de gestion – comptes – budget".casefold():
            continue
        if not file_url or "rapport-de-gestion" not in unquote(file_url).casefold() or file_url in seen:
            continue
        seen.add(file_url)
        filename = legacy.safe_filename(file_url)
        normalized_subject = legacy.normalize(subject)
        records.append({
            "commune": "La Tour-de-Peilz",
            "document_family": "annual_report",
            "category": "rapport_gestion",
            "document_role": "combined_management_report_commission_response",
            "title": f"Rapport de gestion {listing_year}",
            ...
            "management_year": int(listing_year),
            "period_start": f"{listing_year}-01-01",
            "period_end": f"{listing_year}-12-31",
            "listing_contains_commission_report": "commission de gestion" in normalized_subject,
            "listing_contains_municipal_response": "reponse de la municipalite" in normalized_subject,
...
def fetch_page(page: int, session: requests.Session | None = None) -> dict:
    ...
    response = client.get(
        ENDPOINT,
        params={
            "searchdoc": "Rapport de gestion", "categories[]": CATEGORY_ID,
            "sorting": "rang", "direction": "DESC", "page": page, "qty": PAGE_SIZE,
        },
        headers=HEADERS,
        timeout=30,
    )
...
def collect_items(session: requests.Session | None = None) -> tuple[list[dict], dict]:
    ...
    items = sorted(by_url.values(), key=lambda item: item["management_year"])
    diagnostics = {
        "endpoint_rows_all_years": int(first["rows"]),
        "pages_fetched": pages,
        "reports_2021_2026": len(items),
        "unique_pdf_urls": len(by_url),
        "missing_management_year": sum(not item.get("management_year") for item in items),
        "with_commission_report": sum(item["listing_contains_commission_report"] for item in items),
        "with_municipal_response": sum(item["listing_contains_municipal_response"] for item in items),
    }
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Collecte JSON des rapports de gestion 2021-2026")
    ...
    report = {
        "source": {
            ...
            "note": "L'identifiant 4 sert uniquement à la collecte et n'entre pas dans les métadonnées documentaires.",
        },
        "schema": {
            "base_fields": [
            ...
            "management_fields_from_endpoint": ["management_year", "period_start", "period_end"],
            "deferred_to_pdf_audit": [
                "document_date", "decision_date", "components", "commission", "observations_count",
                "wishes_count", "report_sections", "processing",
            ],
            "cleaning_policy": {
                "remove": ["page_numbers", "repeated_running_headers", "repeated_running_footers", "postal_contact_blocks"],
                "preserve": ["department_headings", "table_titles", "table_rows", "commission_observations", "municipal_responses"],
            },
        },
