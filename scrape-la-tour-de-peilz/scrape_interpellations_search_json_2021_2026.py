...
import scrape_interpellations_2021_2026 as legacy
...
YEARS = faceted.YEARS
PAGE_SIZE = 25
HEADERS = {"User-Agent": "AI-Riviera interpellations faceted JSON importer"}
...
def parse_result_html(result_html: str, years: set[str] = YEARS) -> list[dict]:
    ...
    for match in faceted.RESULT_RE.finditer(result_html):
        ...
        if year not in years or not re.match(r"^Interpellation\s", title, flags=re.I):
            continue
        if category.casefold() != "motions, postulats et interpellations":
            continue
        if not pdf_url or "motions-postulats" not in pdf_url.casefold() or pdf_url in seen_urls:
            continue
        ...
        records.append({
            ...
            "political_object_id": f"interpellation-{year}-{legacy.slugify(subject or title or filename)}",
...
def fetch_page(page: int, session: requests.Session | None = None) -> dict:
    ...
    response = client.get(
        ENDPOINT,
        params={
            "searchdoc": "Interpellation", "categories[]": CATEGORY_ID,
...
def collect_items(session: requests.Session | None = None) -> tuple[list[dict], dict]:
    ...
    return items, {
        "endpoint_rows": int(first["rows"]), "pages_fetched": pages,
        "interpellations_2021_2026": len(items), "unique_pdf_urls": len(by_url),
    }


def compare_with_legacy(items: list[dict]) -> dict:
    new_urls = {item["pdf_url"] for item in items}
    old_urls = {item["pdf_url"] for item in legacy.collect_items_legacy_page()}
    return {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collecte JSON des interpellations 2021-2026")
