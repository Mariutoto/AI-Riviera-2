...
import scrape_rapport_comptes_2021_2024 as legacy
...
HEADERS = {"User-Agent": "AI-Riviera rapport-comptes faceted JSON importer"}
...
def parse_result_html(result_html: str, years: set[str] = YEARS) -> list[dict]:
    ...
    for match in faceted.RESULT_RE.finditer(result_html):
        ...
        if listing_year not in years:
            continue
        if source_title.casefold() != "rapport des comptes":
            continue
        if source_category.casefold() != "rapport de gestion – comptes – budget".casefold():
            continue
        if not file_url or "rapport-des-comptes" not in unquote(file_url).casefold() or file_url in seen:
            continue
        seen.add(file_url)
        ...
        records.append({
            ...
            "category": "rapport_comptes",
            "document_role": "annual_accounts_report",
            "title": f"Rapport des comptes {listing_year}",
...
def fetch_page(page: int, session: requests.Session | None = None) -> dict:
    ...
    response = client.get(
        ENDPOINT,
        params={
            "searchdoc": "Rapport des comptes", "categories[]": CATEGORY_ID,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Collecte JSON des rapports des comptes 2021-2026")
    ...
    report = {
        ...
        "schema": {
            ...
            "accounts_fields_from_endpoint": ["fiscal_year", "period_start", "period_end"],
            "deferred_to_pdf_audit": [
                "document_date", "components", "total_charges", "total_revenues",
                "result_surplus_or_deficit", "processing",
            ],
            "cleaning_policy": {
                "remove": ["page_numbers", "repeated_running_headers", "repeated_running_footers", "postal_contact_blocks"],
                "preserve": ["department_headings", "table_titles", "table_rows", "preavis_text"],
            },
        },
