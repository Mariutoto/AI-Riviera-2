...
def iter_political_metadata(root: Path):
    for path in sorted(root.rglob("*.json")):
        if len(path.parts) >= 2 and path.parts[-2] in POLITICAL_CATEGORIES:
            yield path
...
def enrich_file(path: Path, dry_run: bool = False) -> bool:
    ...
    content = text_path.read_text(encoding="utf-8", errors="ignore") if text_path.exists() else ""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Persist enriched metadata for motions, postulats and interpellations.")
    ...
    for path in iter_political_metadata(args.documents_root):
        scanned += 1
        try:
