...
SOURCE_PAGE = "https://www.la-tour-de-peilz.ch/politique/informations-diverses.php"
YEARS = {str(year) for year in range(2021, 2027)}
OUTPUT_ROOT = PROJECT_ROOT / "documents" / "la-tour-de-peilz"
DATA_ROOT = PROJECT_ROOT / "data" / "informations-diverses" / "la-tour-de-peilz"
HEADERS = {"User-Agent": "AI-Riviera informations-diverses importer"}
...
def normalize_pdf_url(page_url: str, href: str) -> str | None:
    ...
    if ".pdf" not in decoded.lower():
        return None
    if "informations-diverses" not in decoded.lower():
        return None
    return full_url
...
def download_and_extract(item: dict) -> dict:
    ...
    target_dir = OUTPUT_ROOT / year / "informations-diverses"
...
def main() -> None:
    ...
    items = collect_items()
    print(f"Found {len(items)} informations diverses for years 2021-2026.")
    ...
    manifest = {
        ...
        "legislature": "2021-2026",
        "source_page": SOURCE_PAGE,
        "scope_note": "La page est structuree par annees; les documents importes sont ceux des rubriques 2021 a 2026 dans informations-diverses. Certains PDF peuvent etre classes sous une annee de liste differente de l'annee presente dans leur URL.",
        "years": sorted(YEARS),
        "documents_downloaded": len(results),
        "failures": failures,
        "documents": results,
    }
    manifest_path = DATA_ROOT / "manifest_informations_diverses_2021_2026.json"
