...
HEADERS = {"User-Agent": "AI-Riviera rapport des comptes importer"}
...
def normalize_pdf_url(page_url: str, href: str) -> str | None:
    ...
    if ".pdf" not in decoded.lower():
        return None
    if "rapport-des-comptes" not in decoded.lower():
        return None
    return full_url
...
def collect_items() -> list[dict]:
    ...
    for li in re.findall(r'<li class="prestation[^>]*>([\s\S]*?)</li>', page_html, flags=re.I):
        ...
        year = year_from_text(f"{title} {href_match.group(1)}")
        ...
        items_by_url[pdf_url] = {
            ...
            "type": "rapport_des_comptes",
            "document_type": "rapport_des_comptes",
            "year": year,
            "category": "rapport-des-comptes",
            "legislature": "2021-2026",
            "title": f"Rapport des comptes {year}",
            "filename": filename,
            "pdf_url": pdf_url,
...
def download_and_extract(item: dict) -> dict:
    ...
    target_dir = OUTPUT_ROOT / year / "rapport-des-comptes"
...
def main() -> None:
    ...
    items = collect_items()
    print(f"Found {len(items)} rapport des comptes documents for 2021-2024.")
    ...
    manifest = {
        ...
        "legislature": "2021-2026",
        "source_page": SOURCE_PAGE,
        "scope_note": "Rapports des comptes 2021-2024 de la Commune de La Tour-de-Peilz.",
        "years": sorted(YEARS),
        "documents_downloaded": len(results),
        "failures": failures,
        "documents": results,
    }
    manifest_path = DATA_ROOT / "manifest_rapports_comptes_2021_2024.json"
