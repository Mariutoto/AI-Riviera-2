...
def ensure_schema() -> None:
    ...
    with _connect() as connection:
        ...
        connection.commit()
    vector_search_ready.cache_clear()
    ...
    try:
        with _connect() as connection:
            with connection.cursor() as cursor:
                ...
                row = cursor.fetchone()
        return bool(row and row["has_vector_column"])
    except Exception as exc:
        record_diagnostic("postgres", "Postgres vector readiness check failed", exc)
        return False
...
def _filter_sql(filters: dict[str, Any], alias: str = "dc") -> tuple[str, list[Any]]:
    filter_where = []
    filter_params: list[Any] = []
    if filters.get("city"):
        filter_where.append(f"LOWER({alias}.city) = LOWER(%s)")
        filter_params.append(filters["city"])
    if filters.get("doc_type"):
        filter_where.append(f"LOWER({alias}.doc_type) = LOWER(%s)")
    ...
    return (f" AND {' AND '.join(filter_where)}" if filter_where else ""), filter_params
...
def search_chunks(query: str, tokens: list[str], limit: int = 10, filters: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    if filters_sql:
        like_where.append(filters_sql[5:])
...
def search_vector_chunks(
    ...
    if not query_embedding or not vector_search_ready():
        return []
    ...
    sql = f"""
        SELECT
            ...
            dc.city,
            d.source_path,
            (1 - (dc.embedding_vector <=> %s::vector)) * 100 AS score
        FROM document_chunks dc
        JOIN documents d ON d.id = dc.document_id
        ...
        LIMIT %s
    """
    params = [vector_literal, *filter_params, vector_literal, max(limit * 8, 50)]
    try:
        with _connect() as connection:
            with connection.cursor() as cursor:
...
def insert_chunks(connection, rows: list[dict[str, Any]]) -> None:
    ...
    with connection.cursor() as cursor:
        has_vector_column = vector_search_ready()
        columns_sql = """
        ...
        if has_vector_column:
            columns_sql += ", embedding_vector"
            values_sql += ", %(embedding_vector)s::vector"
            update_sql += ", embedding_vector = EXCLUDED.embedding_vector"

        cursor.executemany(
            f"""
            INSERT INTO document_chunks ({columns_sql})
            VALUES ({values_sql})
            ...
            [
                {
                    **row,
                    ...
                    "embedding_vector": _vector_literal(row["embedding"]),
