...
def main() -> None:
    ...
    with psycopg.connect(
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                "count(*) FILTER "
                "(WHERE document_role='interpellation_text') "
                "AS original_interpellations, "
                "count(*) FILTER "
                "(WHERE document_role='municipal_response') "
                "AS response_documents, "
                "count(*) FILTER (WHERE document_role='municipal_response' "
                "AND metadata->'additional_metadata'->'source_tracking'"
                "->>'source_kind'='official_response_pdf') "
                "AS response_pdfs "
                "FROM documents "
                "WHERE document_id LIKE 'montreux_interpellation_%%'"
            )
            ...
            cursor.execute(
                ...
                "->'interpellation_metadata'->>'object_id' AS object_id, "
                "metadata->'additional_metadata'"
                "->'interpellation_metadata'->>'object_title' AS title, "
                "(metadata->'additional_metadata'"
                "->'interpellation_metadata'->>'has_response')::boolean "
                "AS has_response "
                "FROM documents "
                "WHERE document_id LIKE 'montreux_interpellation_%%' "
                "ORDER BY object_id"
            ...
            cursor.execute(
                ...
                "FROM documents d "
                "WHERE d.document_id LIKE "
                "'montreux_interpellation_%%'"
    ...
    all_rows = pilot_v2_store.aggregate_authors(
        {"city": "Montreux", "doc_type": "interpellations"}
    )
    answered = pilot_v2_store.answered_interpellations(
        {"city": "Montreux"}
    )
    answered_2025 = pilot_v2_store.answered_interpellations(
        {"city": "Montreux", "response_year": "2025"}
    )
    retrieval_specs = [
        (
            "Ascenseur Coteau-Muraz",
            "montreux_interpellation_2706_",
        ),
        (
            "projecteurs installations sportives",
            "montreux_interpellation_2918_",
        ),
        (
            "giratoire de Brent accès mobilité",
            "montreux_interpellation_",
        ),
        (
            "logement abordable Montreux",
            "montreux_interpellation_",
        ),
    ]
    retrieval = []
        ...
        retrieval.append(
            {
                ...
                "matched": any(
                    row.startswith(expected_prefix)
    ...
    database = {
        ...
        **dict(chunk_counts),
        ...
        "objects_with_response": sum(
            bool(row["has_response"]) for row in objects
        ),
        "objects_without_response": sum(
            not bool(row["has_response"]) for row in objects
        ),
        "broken_relationship_links": broken_links,
        "aggregate_originals": len(all_rows),
        "answered_interpellations": len(answered),
        "answered_in_2025": len(answered_2025),
        "answered_with_source_url": sum(
            bool(row.get("response_url")) for row in answered
        ),
    }
    expected = {
        "documents": 307,
        "original_interpellations": 155,
        "response_documents": 152,
        "response_pdfs": 66,
        "chunks": 666,
        "vectors": 666,
        "min_dimension": 1024,
        "max_dimension": 1024,
        "political_objects": 155,
        "objects_with_response": 152,
        "objects_without_response": 3,
        "broken_relationship_links": 0,
        "aggregate_originals": 155,
        "answered_interpellations": 152,
        "answered_in_2025": 24,
        "answered_with_source_url": 152,
    }
    failures = [
