...
def extract_pdf(document: dict) -> tuple[str, dict]:
    ...
    if len(text) < 120:
        text = clean_text(
            f"Pièce jointe officielle au postulat : {document['source_title']}\n"
            f"Nom du fichier : {document['filename']}\n"
...
def object_text(item: dict) -> str:
    authors = ", ".join(
        " ".join(part for part in [a.get("civility", ""), a.get("name", "")] if part)
        + (f" ({a['party']})" if a.get("party") else "")
        for a in item["authors"]
    )
    return clean_text(
        f"Postulat : {item['title']}\n\n"
        f"Auteur ou autrice : {authors or item['author_text']}\n"
...
def metadata_record(spec: dict, objects: dict[str, dict]) -> dict:
    ...
    specific = {
        ...
        "response_date": primary.get("response_date") or None,
        "has_response": primary["has_response"],
        "has_dedicated_response": bool(primary["response_document_ids"]),
    ...
    original_ids = [
        f"montreux_postulate_{item['source_object_id']}_text" for item in linked
    ]
    related_ids = sorted(set(original_ids + response_ids + report_ids) - {spec["document_id"]})
    return {
        ...
        "source_tracking": {
            "source_collection": "Conseil communal de Montreux",
            "source_kind": spec["source_kind"],
            "source_download_id": spec.get("download_id", ""),
            "source_download_aliases": spec.get("source_download_aliases", []),
        },
        "processing": {
...
def main() -> None:
    ...
    if len(inventory["objects"]) != 49:
        raise SystemExit(f"Périmètre inattendu: {len(inventory['objects'])} postulats")
    ...
    for item in inventory["objects"]:
        ...
        document_id = f"montreux_postulate_{item['source_object_id']}_text"
        specs.append(
            {
    ...
    for document in inventory["documents"]:
        ...
        title = (
            "Réponse municipale — " + " / ".join(linked_titles)
        ...
        specs.append(
            {
                **document,
                ...
                "source_kind": (
                    "official_municipal_response_pdf"
                    if document["document_role"] == "municipal_response"
                    else "official_consideration_report_pdf"
                ),
    ...
    summary = {
        ...
        "documents": len(specs),
        "original_postulates": sum(s["document_role"] == "postulate_text" for s in specs),
    ...
    audit = {
        "schema_version": "montreux-postulates-general-audit-v1",
    ...
    (OUTPUT_DIR / "index.html").write_text(
        "<!doctype html><html lang='fr'><meta charset='utf-8'><body>"
        "<h1>Postulats de Montreux — législature 2021–2026</h1>"
        f"<pre>{html.escape(json.dumps(summary, ensure_ascii=False, indent=2))}</pre>"
        f"<table><tr><th>Postulat</th><th>Dépôt</th><th>Réponse</th></tr>{rows}</table></body></html>",
        encoding="utf-8",
    )
