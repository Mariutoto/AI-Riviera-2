...
def extract_pdf(document: dict) -> tuple[str, dict]:
    ...
    if len(text) < 120:
        text = clean_text(
            f"Pièce jointe officielle à l'interpellation : {document['source_title']}\n"
...
def object_text(item: dict) -> str:
    ...
    return clean_text(
        ...
        "seule la réponse municipale est disponible."
    )


def role_component(role: str) -> str:
    return {
        "interpellation_text": "interpellation_text",
...
def main() -> None:
    ...
    if len(inventory["objects"]) != 51:
        raise SystemExit(f"Périmètre inattendu: {len(inventory['objects'])} interpellations")
    ...
    for document in inventory["documents"]:
        if document["document_role"] != "interpellation_text":
            continue
    ...
    for item in inventory["objects"]:
        ...
        document_id = f"blonay-saint-legier_interpellation_{item['source_object_id']}_text"
    ...
    summary = {
        ...
        "documents": len(specs),
        "original_interpellations": sum(s["document_role"] == "interpellation_text" for s in specs),
        ...
        "objects_with_synthetic_text": sum(
            not item["has_interpellation_text"] for item in objects.values()
        ),
    ...
    audit = {
        "schema_version": "blonay-saint-legier-interpellations-general-audit-v1",
    ...
    (OUTPUT_DIR / "index.html").write_text(
        "<!doctype html><html lang='fr'><meta charset='utf-8'><body>"
        "<h1>Interpellations de Blonay–Saint-Légier — législature 2021–2026</h1>"
        f"<pre>{html.escape(json.dumps(summary, ensure_ascii=False, indent=2))}</pre>"
        f"<table><tr><th>Interpellation</th><th>Dépôt</th><th>Réponse</th></tr>{rows}</table></body></html>",
        encoding="utf-8",
    )
