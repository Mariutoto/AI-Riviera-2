...
import scrape_rapport_comptes_search_json_2021_2026 as endpoint
    ...
    for page_number, page in enumerate(page_texts, 1):
        for raw in page.splitlines():
            ...
            if re.fullmatch(r"!\[[^]]*]\([^)]*\)", normalized):
                ...
                continue
            # Markdown headings and tables are accounting content, even when a
            # table title repeats on many consecutive pages.
            protected = line.startswith("#") or line.startswith("|")
            if normalized and not protected and normalized.casefold() in margin_keys:
                blocks.append({"page": page_number, "text": normalized, "reason": "repeated_margin"})
                continue
            if normalized and not protected and explicit_boilerplate(normalized):
...
COMPONENT_PATTERNS = [
    ("preavis_credits_extra_budgetaires", r"(?im)^\s*(?:#{1,6}\s*)?PR[ÉE]AVIS\s+MUNICIPAL\s+N[°ºO]\s*\d+/\d{4}\s*$"),
    ("compte_fonctionnement", r"(?im)^\s*(?:#{1,6}\s*)?COMPTE\s+DE\s+FONCTIONNEMENT\s*$"),
    ("compte_investissements", r"(?im)^\s*(?:#{1,6}\s*)?COMPTE\s+DES\s+INVESTISSEMENTS\s*$"),
    ("bilan_et_annexes", r"(?im)^\s*(?:#{1,6}\s*)?BILAN(?:\s+AU\s+31\s+D[ÉE]CEMBRE\s+\d{4})?\s*$"),
]


def detect_components(text: str) -> list[str]:
    normalized = normalize(text)
    components = ["preavis_credits_extra_budgetaires"]
    if "rapport des comptes" in normalized and "preavis municipal" in normalized:
        components.append("preavis_rapport_comptes")
    if "compte de fonctionnement" in normalized:
        components.append("compte_fonctionnement")
    if "compte des investissements" in normalized:
        components.append("compte_investissements")
    if re.search(r"(?im)^\s*bilan\s*$", text):
        components.append("bilan_et_annexes")
    return components


def role_from_components(components: list[str]) -> str:
    return "combined_" + "_".join(components) if len(components) > 1 else (components[0] if components else "unknown")
...
def split_components(text: str) -> list[dict]:
    ...
    for component, pattern in COMPONENT_PATTERNS:
        ...
        # The two préavis share one heading pattern; the second occurrence is the
        # "Rapport des comptes" préavis, detected by the paragraph that follows it.
        if component == "preavis_credits_extra_budgetaires" and len(matches) >= 2:
            markers.append((matches[0].start(), "preavis_credits_extra_budgetaires"))
            markers.append((matches[1].start(), "preavis_rapport_comptes"))
            continue
    ...
    if front_matter and parts:
        parts[0]["content"] = front_matter + "\n\n" + parts[0]["content"]
    return parts or [{"component": "rapport_comptes", "content": text}]
...
def build_chunks(metadata: dict, text: str) -> list[dict]:
    base, specific = metadata["document_metadata"], metadata["accounts_metadata"]
    chunks = []
...
def extract_document_date(text: str) -> str | None:
    match = re.search(r"\ble\s+(\d{1,2})\s+([A-Za-zÀ-ÿ]+)\s+(20\d{2})", text, flags=re.I)
    if not match:
        return None
...
def extract_financial_totals(text: str) -> dict:
    totals: dict[str, float | str | None] = {"total_charges": None, "total_revenues": None, "result_surplus_or_deficit": None}
    # The OCR emits the summary as a Markdown table. Restrict each match to one
    # line so later analytical paragraphs cannot overwrite the official totals.
    charges = re.search(r"total\s+de\s+charges\s+de[^\r\n]{0,80}?([0-9][0-9'.]{3,})", text, flags=re.I)
    revenues = re.search(r"total\s+de\s+revenus\s+de[^\r\n]{0,80}?([0-9][0-9'.]{3,})", text, flags=re.I)
    result = re.search(r"exc[ée]dent\s+de\s+(revenus|charges)\s+de[^\r\n]{0,80}?([0-9][0-9'.]{3,})", text, flags=re.I)
    if charges:
        totals["total_charges"] = float(charges.group(1).replace("'", ""))
    if revenues:
        totals["total_revenues"] = float(revenues.group(1).replace("'", ""))
    if result:
        sign = 1 if result.group(1).casefold() == "revenus" else -1
        totals["result_surplus_or_deficit"] = sign * float(result.group(2).replace("'", ""))
    return totals
...
def audit_item(item: dict) -> dict:
    ...
    document_date = extract_document_date(clean_text) or extract_document_date(native_clean_text)
    totals = extract_financial_totals(clean_text)
    native_totals = extract_financial_totals(native_clean_text)
    for field in totals:
        if totals[field] is None:
            totals[field] = native_totals[field]
    ...
    metadata = {
        "document_metadata": {
            ...
            "legislature": item["legislature"], "document_date": document_date,
        ...
        "accounts_metadata": {
            "fiscal_year": item["fiscal_year"], "period_start": item["period_start"],
            "period_end": item["period_end"], "components": components,
            "total_charges": totals["total_charges"], "total_revenues": totals["total_revenues"],
            "result_surplus_or_deficit": totals["result_surplus_or_deficit"],
        },
        "processing": {
    ...
    additional_missing = [field for field in ("fiscal_year", "period_start", "period_end") if metadata["accounts_metadata"].get(field) in (None, "")]
    warnings = []
    if needs_ocr:
        warnings.append("ocr_required")
    if totals["total_charges"] is None or totals["total_revenues"] is None:
        warnings.append("financial_totals_not_detected")
    elif totals["result_surplus_or_deficit"] is not None:
        calculated_result = totals["total_revenues"] - totals["total_charges"]
        if abs(calculated_result - totals["result_surplus_or_deficit"]) > 0.02:
            warnings.append("financial_totals_inconsistent")
    ...
    return {
        "document_id": document_id, "title": item["title"], "fiscal_year": item["fiscal_year"],
...
def audit_html(records: list[dict]) -> str:
    ...
    for record in records:
        ...
        specific, processing = metadata["accounts_metadata"], metadata["processing"]
        rows.append(f"""<tr class='{css}'><td>{record['fiscal_year']}</td><td><a href='{html.escape(record['file_url'])}'>{html.escape(record['title'])}</a></td><td>{html.escape(', '.join(record['components']))}</td><td>{'À vérifier' if css == 'review' else 'Complet'}<br>{html.escape(', '.join(record['warnings']))}</td><td>{html.escape(', '.join(audit['base_missing']) or 'Aucun')}</td><td>{html.escape(', '.join(audit['additional_missing']) or 'Aucun')}</td><td>Charges : {format_amount(specific['total_charges'])}<br>Revenus : {format_amount(specific['total_revenues'])}<br>Résultat : {format_amount(specific['result_surplus_or_deficit'])}</td><td>{processing['header_footer_cleaning']['raw_words']} → {processing['header_footer_cleaning']['clean_words']} mots<br>{processing['header_footer_cleaning']['removed_blocks']} blocs retirés</td><td>{html.escape(processing['selected_text']['method'])}<br>Couverture {record['coverage']:.0%}</td><td>{record['chunks']} chunks<br>{record['green_chunks']} verts, {record['yellow_chunks']} jaunes, {record['red_chunks']} rouges</td><td><a href='metadata/{record['document_id']}.json'>JSON</a> · <a href='clean_text/{record['document_id']}.txt'>Texte</a> · <a href='removed_blocks/{record['document_id']}.json'>Blocs</a> · <a href='details/{record['document_id']}.html'>Contrôles</a> · <a href='chunk_details/{record['document_id']}.html'>Chunks</a></td></tr>""")
    return f"""<!doctype html><html lang='fr'><head><meta charset='utf-8'><title>Audit — rapports des comptes</title><style>{CSS}</style></head><body><h1>Audit des rapports des comptes — {len(records)} documents</h1><p>Source JSON officielle. Nettoyage des marges répétées sans suppression des titres de services ou tableaux. <a href='chunks_audit.html'><strong>Voir l'audit global des chunks</strong></a>.</p><table><thead><tr><th>Exercice</th><th>Document</th><th>Composants</th><th>État</th><th>Manque base</th><th>Manque additionnel</th><th>Résultat</th><th>Nettoyage</th><th>OCR</th><th>Chunks</th><th>Fichiers</th></tr></thead><tbody>{''.join(rows)}</tbody></table></body></html>"""


def chunks_audit_html(records: list[dict]) -> str:
    rows = "".join(f"<tr class='{'red' if r['red_chunks'] else 'yellow' if r['yellow_chunks'] else 'green'}'><td>{r['fiscal_year']}</td><td><a href='chunk_details/{r['document_id']}.html'>{html.escape(r['title'])}</a></td><td>{r['chunks']}</td><td>{r['green_chunks']}</td><td>{r['yellow_chunks']}</td><td>{r['red_chunks']}</td><td>{html.escape(', '.join(r['components']))}</td></tr>" for r in records)
    return f"""<!doctype html><html lang='fr'><head><meta charset='utf-8'><title>Chunks — rapports des comptes</title><style>{CSS}</style></head><body><p><a href='audit.html'>← Audit principal</a></p><h1>Audit global des chunks — {len(records)} rapports</h1><table><tr><th>Exercice</th><th>Document</th><th>Chunks</th><th>Verts</th><th>Jaunes</th><th>Rouges</th><th>Composants</th></tr>{rows}</table></body></html>"""
