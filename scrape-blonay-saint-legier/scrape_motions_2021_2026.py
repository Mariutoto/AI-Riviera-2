...
POSTULATS_SCRIPT = PROJECT_ROOT / "scrape-blonay-saint-legier" / "scrape_postulats_2021_2026.py"
POSTULATS_SPEC = importlib.util.spec_from_file_location("blonay_saint_legier_postulate_source", POSTULATS_SCRIPT)
...
OBJECT_CATEGORY = "Motion"

base.parse_authors = postulats.parse_authors_with_roles
...
def build_inventory(output_dir: Path) -> dict:
    ...
    motions = [row for row in index_rows if row["category"] == OBJECT_CATEGORY]
    selected_ids = sorted(row["source_object_id"] for row in motions)
    ...
    for row in sorted(motions, key=lambda item: item["source_object_id"]):
        ...
        for attachment in detail["attachments"]:
            ...
            document_id = (
                f"blonay-saint-legier_motion_{source_id}_{suffix}_"
    ...
    for item in objects:
        item["has_motion_text"] = any(
            document["political_object_ids"] == [item["object_id"]]
            and document["document_role"] == "motion_text"
            for document in documents
        )
    ...
    diagnostics = {
        ...
        "objects_missing_original_text": sum(not item["has_motion_text"] for item in objects),
        ...
        "objects_answered_status_without_response_document": sum(
            item["status_normalized"] == "answered" and not item["has_response"]
            for item in objects
        ),
    }
    return {
        "schema_version": "blonay-saint-legier-motions-inventory-v1",
        ...
        "automatic_detection_notes": {
            ...
            "author_field": (
                "the 'Auteur' field can list commission members instead of, or alongside, the "
                "original depositor once a commission has been formed; each person's role "
                "label from the site is preserved as-is"
            ),
        },
...
def main() -> None:
    root = PROJECT_ROOT / "audit-blonay-saint-legier" / "motions-2021-2026"
    parser = argparse.ArgumentParser(
        description="Scrape les motions de Blonay-Saint-Légier 2021-2026"
    )
